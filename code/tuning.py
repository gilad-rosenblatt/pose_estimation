...
def run_grid_scan(model_filename):
    ...
    scorer = DetectionEvaluator(model_filename=model_filename)

    # Make sure a scores folder exists.
    if not os.path.exists(DetectionEvaluator.SCORES_DIR):
        os.makedirs(DetectionEvaluator.SCORES_DIR)
    ...
    for cls_threshold, nms_threshold in itertools.product(cls_thresholds, nms_thresholds):
        ...
        this_stats = DetectionEvaluator.score(this_filename)
        stats.append(dict(
    ...
    with open(os.path.join(DetectionEvaluator.SCORES_DIR, "threshold_tuning.json"), "w") as out_file:
...
def get_best_hypers(category="map_50"):
    ...
    with open(os.path.join(DetectionEvaluator.SCORES_DIR, "threshold_tuning.json"), "r") as in_file:
